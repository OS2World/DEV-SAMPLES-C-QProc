			      ...
			      ((endln < end)||(end == NULL)))
			{
			    ...
			    if (n > 0) {
				    if (pdev->d_mode != 'U')
					    ...
					    memcpyf(line+pos, buf+i, new-pos);
...
PDEVICESPL pdev;
{
    ...
    strncpyf(pdev->d_szUser, pprj->pszUserName, UNLEN);
    ...
    if (stricmpf(separator, DEFAULT_SEPARATOR) == 0) {
        pdev->d_handle = NULL_HFILE;
    } else {
        if ((isascii(*separator) && *(separator+1) == ':' ) ) {
            strcpyf(buf1, separator);
        } else {
            PSZ psz;
            ...
            psz = buf1;
            if (pdev->d_pszPath)
                psz = strcpyf(buf1, pdev->d_pszPath);

            SafeStrcpy(EndStrcat(psz, "\\"), separator);
        }
