...
BOOL near SplLoadProc( hModule, cbHeap )
HMODULE hModule;
USHORT cbHeap;
{
    ...
    if (!hSplModule) {
        ...
        memsetf( &semPMPRINT, 0, sizeof( FSRSEM ));
    ...
    /* For each process that attaches to us, install an exitlist handler */

    DosExitList( EXLST_ADD, (PFNEXITLIST)SplExitListProc );
